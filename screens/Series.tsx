...
function SeriesScreen({navigation}: any): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    retrieveData('hiddenCategoriesSeries').then(data => {
      const hiddenCategories = JSON.parse(data || '[]');
      setHiddenCategories(
        Array.isArray(hiddenCategories) ? hiddenCategories : [],
      );
    });
  }, [focused]);
  ...
  const toggleEditMode = () => {
    if (editMode) {
      storeData('hiddenCategoriesSeries', [...hiddenCategories]);
    }
  ...
  const showCategory = (id: string) => {
    setHiddenCategories(prevHiddenCategories =>
      prevHiddenCategories.filter(categoryId => categoryId !== id),
    );
  };
  ...
  const hiddenCategoriesList = categories.filter(category =>
          ...
          <View className="flex-1 flex-col justify-center items-center w-full">
            {visibleCategories.map(category => {
              ...
              return (
                <View
                  key={category.id}
                  className="rounded-lg w-10/12 h-10 m-2 p-2 flex-row justify-between items-center"
                  ...
                  {editMode && (
                    <TouchableOpacity
                      onPress={() => hideCategory(category.id)}
            ...
            {hiddenCategoriesList.length > 0 && editMode && (
              <View style={{width: '100%', alignItems: 'center'}}>
                <Text
                  style={{color: theme.primary, marginVertical: 10}}
                ...
                {hiddenCategoriesList.map(category => {
                  ...
                  return (
                    <View
                      ...
                      {editMode && (
                        <TouchableOpacity
                          onPress={() => showCategory(category.id)}
                          style={{position: 'absolute', right: 3}}>
                          <MaterialCommunityIcons
                            name="undo"
                            size={24}
