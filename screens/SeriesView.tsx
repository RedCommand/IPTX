  ...
  return (
    <SafeAreaView style={{backgroundColor: theme.background, flex: 1}}>
      <ImageBackground
        ...
        source={{uri: seriesInfo?.background}}
        imageStyle={{opacity: isDarkMode ? 0.15 : 0.25}}
        resizeMode="cover">
        <View className="flex flex-row justify-between">
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              var currentBucketList: string[] = await JSON.parse(
                bucket || '[]',
              );
              var currentItem = 'series-' + seriesInfo?.stream_id;
              if (
                currentBucketList &&
                currentBucketList.includes(currentItem)
              ) {
              ...
              await storeData('bucket-' + profile, currentBucketList);
              ToastAndroid.show('Added to Bucket List', ToastAndroid.LONG);
            }}>
        ...
        <ScrollView
          ...
          <View className="flex flex-row items-center justify-between w-10/12 pb-4">
            <Text style={{color: theme.text}} className="text-lg">
              {seriesInfo?.episodes.length} Episodes
            </Text>
            <Text style={{color: theme.primary}} className="text-xl">
              •
            </Text>
            <Text style={{color: theme.text}} className="text-lg">
              TMDb {seriesInfo?.rating}/10
            </Text>
            <Text style={{color: theme.primary}} className="text-xl">
              •
            </Text>
            <Text style={{color: theme.text}} className="text-lg">
              {seriesInfo?.release_date?.split('-')[0]}
            </Text>
          </View>

          <Text
            style={{color: theme.secondary}}
            className="pb-1 font-extrabold text-xl w-11/12">
            Plot
          </Text>
          <Text style={{color: theme.text}} className="pb-4 w-11/12">
            {seriesInfo?.plot}
          </Text>
          ...
          <View className="flex flex-row items-center justify-between w-10/12 pb-4">
            <SelectDropdown
              ...
              renderButton={(selectedItem, isOpened) => {
                return (
                  <View
                    className="w-full h-12 justify-center items-center px-3 flex flex-row rounded-lg "
              ...
              renderItem={(item, index, isSelected) => {
                return (
                  <View className="items-center">
                    <View
                      className="h-12 justify-center items-center px-3 flex flex-row rounded-lg"
                    ...
                    {index !== seasons.length - 1 && (
                      <View
                        className="h-0.5 w-11/12"
                        style={{backgroundColor: theme.primary}}></View>
                    )}
                  </View>
                );
              }}
          ...
          {seriesInfo?.episodes
            ...
            .map((episode, index) => {
              return (
                <TouchableOpacity
                  key={index}
                  className="flex flex-row items-center justify-center w-11/12 p-2"
                  onPress={async () => {
                  ...
                  <View
                    className="flex-row w-11/12 h-20 m-2 "
                    style={{backgroundColor: theme.card, borderRadius: 10}}>
                    <Image
                      source={{uri: episode.background}}
                      style={{borderRadius: 10}}
                      defaultSource={{
                        uri:
                          seriesInfo.background != ''
                            ? seriesInfo.background
                            : seriesInfo.cover,
                      }}
                    ...
                    <View className="flex flex-col ml-2 justify-center w-full">
                      ...
                      <Text
                        numberOfLines={1}
                        style={{color: theme.text}}
                        className="text-sm w-[70%] self-start">
                        {episode.title}
                      </Text>
                      <Text
                        numberOfLines={1}
        ...
        <View className="bottom-0 mb-3 absolute w-full gap-2 px-2 items-center flex flex-row">
          <Button
            ...
            onPress={async () => {
              Linking.openURL(
                `https://www.themoviedb.org/tv/${seriesInfo?.tmdb_id}`,
              );
            }}>
